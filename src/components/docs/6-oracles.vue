<template>
  <div>
    ...
    <p>
      ...
      ability of listing dozens of markets, each requiring it&#39;s own Oracle with it&#39;s own set
      of rules, <strong>we created a customizable Oracle subsystem</strong> as part of the
      <strong>rLending Protocol</strong>.
    </p>
    <hr />
    <h2 id="architecture-">Architecture:</h2>
    <p>
      <strong>rLending</strong>&#39;s Oracle subsystem is comprised by a set of
    ...
    <p>
      Since there is no standard on how Oracles report their values and each market listed in the
      protocol requires an independant interaction with an oracle that provides its current exchange
      rate, we included an &quot;Adapter&quot; layer that acts as a middle-man between the
      <em>PriceProxyOracle</em> and the actual Oracle Contracts providing rates for each market.
    </p>
    <p>
      In order to preserve decentralization, <strong>rLending</strong> does not manage any of the
      oracles the protocol is currently using. We will talk in detail about this in the next section
