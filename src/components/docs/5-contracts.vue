    ...
    <v-container>
      <v-row class="d-flex align-center justify-left">
        ...
        <p>
          ...
          cTokens, she will do so using the cToken contract.
        </p>
        <p>
        ...
        <p>
          ...
          block, which is about every 30 seconds. This means that the exchange rate of underlying to
          cToken increases over time. There is no minimum or maximum amount of time that suppliers
          need to keep their asset in the protocol. See the varying exchange rate for each cToken by
          clicking on one at (rlending)[rlending.com] markets.
        </p>
        <p>&nbsp;</p>
        <hr />
        ...
        <p>
          ...
          user is required to maintain and whether (and by how much) a user can be liquidated. Each
          time a user interacts with a cToken, the <strong>Comptroller</strong> is asked to approve
          or deny the transaction.
        </p>
        <p>
          The <strong>Comptroller</strong> maps user balances to prices (via the Price Oracle) to
          risk weights (called Collateral Factors) to make its determinations. Users explicitly list
          which assets they would like included in their risk scoring, by calling Enter Markets and
          Exit Market (toggling each market in the Supply / Borrow tab).
        </p>
        <p>
